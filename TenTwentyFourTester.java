...
public class TenTwentyFourTester {
	...
	public void resetShouldShowTwoValue()
	{
		...
		assertTrue (c1.getRow() != c2.getRow() || c1.getColumn() != c2.getColumn());
		assertTrue (isPowerTwo(c1.getValue()));
		assertTrue (isPowerTwo(c2.getValue()));
	}
	...
	private void swiper (NumberSlider cm, SlideDirection dir, String chPat, int[][] before, int[][] after)
	{
		...
		for (Cell c : out) {
			if (c.getRow() < 0 || c.getRow() >= after.length || c.getColumn() < 0 || c.getColumn() >= after[0].length)
				fail("game1024.Cell index (" + c.getRow() + "," + c.getColumn() + " is out of bound");
	...
	private void noMoveSwiper (NumberSlider cm, SlideDirection dir, int[][] mat)
	{
		...
		for (Cell c : out) {
			if (c.getRow() < 0 || c.getRow() >= mat.length || c.getColumn() < 0 || c.getColumn() >= mat[0].length)
				fail("game1024.Cell index (" + c.getRow() + "," + c.getColumn() + " is out of bound");
			assertTrue("getNonEmptyTiles() should return only non-zero cells", c.getValue() > 0);
			assertTrue(action + " (" + c.getRow() + "," + c.getColumn() + ") should be zero",
					mat[c.getRow()][c.getColumn()] != 0);
			actual[c.getRow()][c.getColumn()] = c.getValue();
		}
	...
	private final static String[] _NOT_OVER = {
